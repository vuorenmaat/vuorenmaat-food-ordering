...
def registerNext():
    ...
    if request.method == "GET":
        ...
        customercheck = Customer.query.filter(
            and_(Customer.cmail == cmail, Customer.cpassword == cpassword)
        ).first()
        ...
        if customercheck:
            ...
            return render_template(
                "userlogin.html",
                cusmsg="Registered Succcessfully...! \n Please Login To Continue",
            )
...
def userhome1():
    ...
    return render_template(
        "userhome.html", cusname=customer.cname, restadmin=Restadmin.query.all()
    )
...
def userorders():
    ...
    return render_template(
        "userorders.html",
        cusname=customer.cname,
        restadmin=customer.query.all(),
        myorders=myorders,
    )
...
def showuserprofile():
    ...
    # db.session.commit()
    return render_template(
        "showuserprofile.html", cusname=customer.cname, cusinfo=customer
    )
...
def updateuserprofileNext():
    ...
    db.session.commit()
    return render_template(
        "updateuserprofile.html", cmsg="Passsword Updated Succcessfully...!"
    )
...
def restloginNext():
    ...
    if request.method == "GET":
        ...
        restadmin = Restadmin.query.filter(
            and_(Restadmin.rmail == rmail, Restadmin.rpassword == rpassword)
        ).first()
...
def restacceptedorders():
    ...
    restadmin = Restadmin.query.filter(Restadmin.rmail == rmail).first()
    rid = restadmin.rid
    myorders = Orders.query.filter(
        and_(Orders.rid == rid, Orders.ostatus == "accepted")
    )
    restadmin = Restadmin.query.filter(Restadmin.rmail == rmail).first()
...
def additemNext():
    ...
    items = Items(
        iname=request.form["iname"], iprice=request.form["iprice"], rid=restid
    )
    db.session.add(items)
    db.session.commit()
...
def updateitemNext():
    ...
    if item:
        ...
        return render_template(
            "updateitem.html", imsg="Error! Item id does not belong to you..! "
        )
...
def deleteitemNext():
    ...
    if item:
        ...
        # return redirect(url_for('updateitem'))
        return render_template(
            "removeitem.html", imsg="Error! Item id does not belong to you..! "
        )
...
def updaterestprofileNext():
    ...
    db.session.commit()
    return render_template(
        "updaterestprofile.html", rmsg="Passsword Updated Succcessfully...!"
    )
...
def payment():
    ...
    return render_template(
        "payment.html", x=x, tprice=tprice, rname=rname, items=items, rid=rid
    )
...
def submitorder():
    ...
    orders = Orders(
        cid=customer.cid, rid=rid, items=items, tprice=tprice, ostatus=ostatus
    )
    if orders:
...
def adminloginNext():
    ...
    if request.method == "GET":
        ...
        diginadmin = Diginadmin.query.filter(
            and_(Diginadmin.amail == amail, Diginadmin.apassword == apassword)
        ).first()
...
def restregisterbyadmin():
    ...
    restadmin = Restadmin.query.filter(
        or_(Restadmin.rmail == rmail, Restadmin.rmobile == rmobile)
    ).first()

    if restadmin:
        # return redirect(url_for('adminHome1'))
        return render_template(
            "adminhome.html", admsg="Restaurant Already Registered...!"
        )
        ...
        # return redirect(url_for('adminHome1'))
        return render_template(
            "adminhome.html", ssmsg="Restaurant Registered Succcessfully...!"
        )
